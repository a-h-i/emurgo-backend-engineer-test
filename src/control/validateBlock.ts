...
export async function isValidHeight(block: Block, trx: Transaction<DB>) {
    const result = await trx.selectFrom('block')
        .select((eb) => {
            return eb.fn.coalesce((eb) => eb.fn.max<bigint>('height'), sql`0`).as('maxHeight');
        }).executeTakeFirstOrThrow();
    return BigInt(block.height) === BigInt(result.maxHeight) + BigInt(1);
}

export async function areOutputsAndInputsMatching(block: Block, trx: Transaction<DB>) {
    if (block.height === 1) {
        // pass for seed block
        return true;
    }
    ...
    for (const tx of block.transactions) {
        ...
        const matchingOutputs = await trx.selectFrom('output')
            .select('value')
        ...
        const matchingOutputsSum = matchingOutputs.map(output => BigInt(output.value)).reduce((acc, value) => acc + value, BigInt(0));
        const outputsSum = tx.outputs.map(output => BigInt(output.value)).reduce((acc, value) => acc + value, BigInt(0));
