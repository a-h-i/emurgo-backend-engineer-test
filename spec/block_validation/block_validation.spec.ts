...
describe("block height validation", () => {
    ...
    test("Empty database valid height", async () => {
        ...
        const isValid = await db.transaction().execute(trx => isValidHeight(block, trx));
    ...
    test("Empty database invalid height", async () => {
        ...
        let isValid = await db.transaction().execute(trx => isValidHeight(block, trx));
        expect(isValid).toBe(false);
        block.height = 2;
        isValid = await db.transaction().execute(trx => isValidHeight(block, trx));
        expect(isValid).toBe(false);
...
describe("block outputs and inputs matching", () => {
    ...
    test('seed block valid', async () => {
        ...
        const isValid = await db.transaction().execute(trx => areOutputsAndInputsMatching(block, trx));
        expect(isValid).toBe(true);
    });

    test('has matching outputs and inputs', async () => {
        await db.insertInto('block').values({id: "block1", height: 1}).execute();
        await db.insertInto('transactions').values({id: "tx1", block_id: "block1"}).execute();
        await db.insertInto('address').values({id: "address1", balance: 100}).execute();
        await db.insertInto('output').values({
            transaction_id: "tx1",
            address_id: "address1",
            value: 100,
            index: 0
